...
## [1.5.2](https://github.com/tibuntu/homeassistant-kubernetes/compare/v1.5.1...v1.5.2) (2026-07-30)
...
**Ingresses in the dashboard panel.** A new **Network** tab lists every Ingress with its class, backing service, TLS status, age — and clickable URLs derived from each Ingress's rules and TLS configuration, so you can jump straight to your services. Comes with an *Ingresses Count* sensor on the cluster device, and Ingresses are covered by the experimental Watch API for real-time updates.

**Live panel updates.** The panel now subscribes to cluster updates over the websocket, so every tab refreshes the moment new data arrives instead of waiting for the next 30-second poll.

> **Upgrading — RBAC:** Ingress support needs read access to `ingresses` in the `networking.k8s.io` API group. Both bundled manifests (**full** and **minimal**) already include the new rule; custom manifests need `get, list` (plus `watch` for the Watch API) added to see Ingresses.
...
* **deps:** update dependency kubernetes to v36.0.2 ([14cee87](https://github.com/tibuntu/homeassistant-kubernetes/commit/14cee87066e25eae0c2b08ef15af5113089bcc4c))
...
* **deps:** update dependency kubernetes to v36 ([5fd00a5](https://github.com/tibuntu/homeassistant-kubernetes/commit/5fd00a54f98367c262ea7a977e25490590d4657d))
...
* **deps-frontend:** lock file maintenance ([d82cc2a](https://github.com/tibuntu/homeassistant-kubernetes/commit/d82cc2a089bdf60c3e388a071a0e5c7abdeaa961))
* **deps-frontend:** lock file maintenance ([1b331b6](https://github.com/tibuntu/homeassistant-kubernetes/commit/1b331b65ae70b9f7247025d67c423f4c0d036092))
* **deps-frontend:** update frontend dependencies to v8.59.2 ([9d3e365](https://github.com/tibuntu/homeassistant-kubernetes/commit/9d3e365eb7091aaf1fe55bf444e586e87a354ad7))
* **deps:** update dependency pytest-homeassistant-custom-component to v0.13.327 ([4dda98a](https://github.com/tibuntu/homeassistant-kubernetes/commit/4dda98ad17ec231148155938ede86a57da2b4c9e))
