...
- **`services.py`** — Three HA services: `scale_workload`, `start_workload`, `stop_workload`. Support targeting multiple entities. Also accepts raw workload names (not just entity IDs) via `_resolve_raw_workload_name` which searches coordinator data to resolve workload types.
