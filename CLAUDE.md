...
- **`websocket_api.py`** — WebSocket API for the sidebar panel. Registers `kubernetes/cluster/overview`, `kubernetes/nodes/list`, and `kubernetes/pods/list` commands that aggregate coordinator data across all config entries. Overview returns cluster health, resource counts, namespace breakdown, and alerts. Nodes/pods list commands return full resource details per cluster.
