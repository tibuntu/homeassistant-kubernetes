...
async function Pe() {
	...
	}] }, e._updateRoutes(), await e.routerOptions.routes.tmp.load(), customElements.get("ha-card") || await customElements.whenDefined("ha-card");
}
var Fe = async () => {
	if (!customElements.get("ha-card")) try {
		await Promise.race([Pe(), new Promise((e, t) => setTimeout(() => t(/* @__PURE__ */ Error(`Timeout waiting for HA elements (${Ne}ms)`)), Ne))]);
	} catch (e) {
...
var V = class extends P {
	...
	firstUpdated(e) {
		this._loadData(), this.pollMs > 0 && (this._startPolling(), document.addEventListener("visibilitychange", this._boundVisibilityHandler)), this.subscribe && this._subscribeUpdates();
	}
	disconnectedCallback() {
		super.disconnectedCallback(), this._stopPolling(), document.removeEventListener("visibilitychange", this._boundVisibilityHandler), this._unsubUpdates?.().catch(() => {}), this._unsubUpdates = void 0, this._updateDebounce &&= (clearTimeout(this._updateDebounce), void 0), this._reloadTimer &&= (clearTimeout(this._reloadTimer), void 0);
	}
	_handleVisibilityChange() {
		document.hidden ? this._stopPolling() : (this._loadData(), this._startPolling());
	}
	_startPolling() {
	...
	async _subscribeUpdates() {
		try {
			let e = await this.hass.connection.subscribeMessage(() => this._scheduleLoad(), { type: "kubernetes/subscribe_updates" });
			if (!this.isConnected) {
				e().catch(() => {});
				return;
			}
			this._unsubUpdates = e;
		} catch {}
	}
	_scheduleLoad() {
		document.hidden || (this._updateDebounce ||= setTimeout(() => {
			this._updateDebounce = void 0, this._loadData();
		}, 1e3));
	}
	_scheduleReload(e) {
		this._reloadTimer && clearTimeout(this._reloadTimer), this._reloadTimer = setTimeout(() => {
			this._reloadTimer = void 0, this._loadData();
		}, e);
	}
	async _loadData() {
		if (!this._loadingInFlight) {
			this._loadingInFlight = !0, this.hasData() || (this._loading = !0), this._error = null;
			try {
...
`, Ve = {
	...
	async fetchData() {
		let e = await this.hass.callWS({ type: "kubernetes/cluster/overview" });
		this._data = e;
	}
	_toggleNamespaces(e) {
	...
	_renderNamespaceSection(e) {
      ...
      <div
        ...
        @keydown=${(t) => {
			(t.key === "Enter" || t.key === " ") && (t.preventDefault(), this._toggleNamespaces(e.entry_id));
		}}
      >
...
`, q = class extends V {
	...
	async fetchData() {
		let e = await this.hass.callWS({ type: "kubernetes/nodes/list" });
		this._data = e;
	}
	_toggleNode(e) {
	...
	_getConditions(e) {
		let t = [];
		return e.memory_pressure && t.push("memory_pressure"), e.disk_pressure && t.push("disk_pressure"), e.pid_pressure && t.push("pid_pressure"), e.network_unavailable && t.push("network_unavailable"), t;
	}
	_getFilteredNodes(e) {
		let t = e;
		if (this._statusFilter !== "all" && (t = t.filter((e) => this._statusFilter === "ready" ? e.status === "Ready" : e.status !== "Ready")), this._searchQuery) {
			let e = this._searchQuery.toLowerCase();
			t = t.filter((t) => t.name.toLowerCase().includes(e) || t.internal_ip.toLowerCase().includes(e) || t.kubelet_version.toLowerCase().includes(e));
		}
		return t;
	}
	async _setSchedulable(e, t, n) {
		let r = `${e}_${t.name}`, i = new Set(this._actionInProgress);
		i.add(r), this._actionInProgress = i;
		try {
			await this.hass.callService("kubernetes", n ? "uncordon_node" : "cordon_node", {
				node_name: t.name,
				entry_id: e
			...
			e.delete(r), this._actionInProgress = e;
		}
	}
	static {
		this.styles = [
	...
	_renderNode(e, t) {
      ...
      <ha-card class="node-card">
        <div
          ...
          @keydown=${(e) => {
			(e.key === "Enter" || e.key === " ") && (e.preventDefault(), this._toggleNode(n));
		}}
        >
          <div class="node-name">
            <ha-icon
          ...
          <button
            ...
            @click=${(n) => {
			n.stopPropagation(), this._setSchedulable(e, t, !t.schedulable);
		}}
          >
            <ha-icon
...
var J = class extends V {
	...
	firstUpdated(e) {
		super.firstUpdated(e), document.addEventListener("click", this._boundCloseMenu);
	}
	disconnectedCallback() {
		super.disconnectedCallback(), document.removeEventListener("click", this._boundCloseMenu);
	}
	async fetchData() {
		let e = await this.hass.callWS({ type: "kubernetes/pods/list" });
		this._data = e;
	}
	_getNamespaces(e) {
		return [...new Set(e.map((e) => e.namespace))].sort();
	}
	_getFilteredPods(e) {
		let t = e;
		if (this._phaseFilter !== "all" && (t = t.filter((e) => e.phase === this._phaseFilter)), this._namespaceFilter !== "all" && (t = t.filter((e) => e.namespace === this._namespaceFilter)), this._searchQuery) {
			let e = this._searchQuery.toLowerCase();
			t = t.filter((t) => t.name.toLowerCase().includes(e) || t.namespace.toLowerCase().includes(e) || t.node_name.toLowerCase().includes(e) || t.owner_name.toLowerCase().includes(e));
		}
		return t = [...t].sort((e, t) => {
			let n, r, i = this._sortField;
			i === "restarts" ? (n = e.restart_count, r = t.restart_count) : i === "age" ? (n = e.creation_timestamp || "", r = t.creation_timestamp || "") : (n = e[i] || "", r = t[i] || "");
			let a = n < r ? -1 : +(n > r);
			return this._sortAsc ? a : -a;
		}), t;
	}
	_handleSort(e) {
		this._sortField === e ? this._sortAsc = !this._sortAsc : (this._sortField = e, this._sortAsc = !0);
	}
	_handleSortKeydown(e, t) {
		(e.key === "Enter" || e.key === " ") && (e.preventDefault(), this._handleSort(t));
	}
	_requestDelete(e, t) {
		this._deleteConfirm = {
			entry_id: e,
	...
	_sortIcon(e) {
		return this._sortField === e ? this._sortAsc ? "mdi:arrow-up" : "mdi:arrow-down" : "";
	}
	static {
		this.styles = [
			K,
	...
	_colVisible(e) {
		return this._visibleColumns.has(e);
	}
	_toggleColumn(e) {
	...
	_renderColumnMenu() {
      ...
      <div class="column-menu-wrapper">
        <button
          ...
          @click=${(e) => {
			e.stopPropagation(), this._columnMenuOpen = !this._columnMenuOpen;
		}}
        >
...
var Ye = [
	...
	_getFilteredIngresses(e) {
		...
		return e.filter((e) => e.name.toLowerCase().includes(t) || e.namespace.toLowerCase().includes(t) || e.rules.some((e) => (e.host || "").toLowerCase().includes(t)));
	}
	_getFilteredServices(e) {
		let t = e;
		if (this._typeFilter !== "all" && (t = t.filter((e) => e.type === this._typeFilter)), this._searchQuery) {
			let e = this._searchQuery.toLowerCase();
			t = t.filter((t) => t.name.toLowerCase().includes(e) || t.namespace.toLowerCase().includes(e) || t.external_ips.some((t) => t.toLowerCase().includes(e)));
		}
		return t;
	}
	_services_of(e) {
		return [...new Set(e.rules.map((e) => e.service_name))].filter(Boolean).join(", ");
	}
	_hasTls(e) {
		return e.tls_hosts.length > 0 || e.urls.some((e) => e.startsWith("https://"));
	}
	_formatPort(e) {
		let t = e.target_port != null && String(e.target_port) !== String(e.port) ? `→${e.target_port}` : "", n = e.node_port ? ` (node ${e.node_port})` : "";
		return `${e.port}${t}/${e.protocol}${n}`;
	}
	static {
		this.styles = [
			H,
...
var X = {
	...
	async fetchData() {
		let e = await this.hass.callWS({ type: "kubernetes/workloads/list" });
		this._data = e;
	}
	_getNamespaces(e) {
		let t = /* @__PURE__ */ new Set();
		for (let n of e.deployments) t.add(n.namespace);
	...
	_handleCategoryKeydown(e, t) {
		(e.key === "Enter" || e.key === " ") && (e.preventDefault(), this._toggleCategory(t));
	}
	_matchesNamespace(e) {
		return this._namespaceFilter === "all" || e === this._namespaceFilter;
	}
	_matchesSearch(e) {
	...
	async _runAction(e, t) {
		let n = new Set(this._actionInProgress);
		n.add(e), this._actionInProgress = n;
		try {
			await t();
		} catch (e) {
			let t = z(e, "Action failed");
			...
			t.delete(e), this._actionInProgress = t;
		}
	}
	_callService(e, t, n) {
		return this._runAction(n, async () => {
			await this.hass.callService("kubernetes", e, t), this._scheduleReload(2e3);
		});
	}
	_setCronJobSuspend(e, t, n, r) {
		return this._runAction(r, async () => {
			await this.hass.callWS({
      ...
      <ha-card class="workload-card">
        <div class="workload-row">
          ...
          <span
            ...
            @keydown=${(e) => {
			(e.key === "Enter" || e.key === " ") && (e.preventDefault(), this._openScaleDialog(n, t.name, t.namespace, t.replicas));
		}}
          >
            ${s}/${t.replicas} ready
          </span>
      ...
      >
        <div class="confirm-dialog" @click=${(e) => e.stopPropagation()}>
          ...
          <div class="scale-controls">
			...
			!isNaN(t) && t >= 0 && (this._scaleValue = t);
		}}
            />
            <button
              class="scale-btn"
...
var Xe = class extends V {
	...
	async fetchData() {
		let e = await this.hass.callWS({ type: "kubernetes/config/list" });
		this._data = e;
	}
	...
	render() {
      ...
      <div class="toolbar">
        <div
          ...
          @keydown=${(e) => {
			(e.key === "Enter" || e.key === " ") && (e.preventDefault(), this._toggleSidebar());
		}}
        >
      ...
      <div class="tab-bar">
        ${Ze.map((e) => T`
            <div
              ...
              @keydown=${(t) => {
			(t.key === "Enter" || t.key === " ") && (t.preventDefault(), this._handleTabChange(e.id));
		}}
            >
