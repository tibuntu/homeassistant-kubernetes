...
Entries using **Prefer in-cluster ServiceAccount token** re-read the projected token file and retry before reporting an authentication failure, so ordinary token rotation never triggers this prompt. If such an entry does reach re-authentication, the projected token is being rejected persistently — submitting a token therefore also turns **Prefer in-cluster ServiceAccount token** off for that entry, so the token you entered is the one actually used. Re-enable the checkbox via **Reconfigure** once the ServiceAccount is healthy again. See [Authentication Issues](TROUBLESHOOTING.md#authentication-issues-401-unauthorized) for how to obtain a fresh token.
...
> **RBAC:** The watch feature requires the service account to have `watch` permission on all monitored resources — granted by the `full` permission set, but **not** by `minimal`. If a watch *connection* keeps failing (network issues, an unreachable API server), the integration raises a repair issue and automatically falls back to regular interval polling, so data stays current. If the ServiceAccount is not allowed to watch a resource (HTTP 403), that stream stops and a **Missing permissions** repair issue names it; when every stream is forbidden (for example with the `minimal` RBAC mode) the integration polls at the regular interval instead. Add the RBAC rule (the chart's `full` mode and `manifests/full/` include every rule the integration needs) and reload the integration to clear it. See the [RBAC guide](RBAC.md) for details.
